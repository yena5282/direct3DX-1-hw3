...
D3D12_RASTERIZER_DESC CShader::CreateRasterizerState()
{
	...
	// -----------------------------------------------------
	// 프리미티브(삼각형)의 내부를 칠할지 말지 변 or 면
	// D3D12_FILL_MODE_WIREFRAME <-> D3D12_FILL_MODE_SOLID;
	d3dRasterizerDesc.FillMode = D3D12_FILL_MODE_SOLID;
	// -----------------------------------------------------
	
	// -----------------------------------------------------
	// 레스터라이저 상태의 컬링모드에대한 은면 유무
	//D3D12_CULL_MODE_BACK <-> D3D12_CULL_MODE_NONE	[+ D3D12_CULL_MODE_FRONT ];
...
void CPlayerShader::CreateShader(ID3D12Device* pd3dDevice, ID3D12RootSignature * pd3dRootSignature)
{
	m_nPipelineStates = 1;
