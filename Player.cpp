...
void CPlayer::Move(DWORD dwDirection, float fDistance, bool bVelocity)
{
	if (dwDirection)
	{
		float kxDelta = 0.0f, kyDelta = 0.0f, kzDelta = 0.0f;
		XMFLOAT3 xmf3Shift = XMFLOAT3(kxDelta, kyDelta, kzDelta);
...
{
	//bVelocity가 참이면 플레이어를 이동하지 않고 속도 벡터를 변경한다. 
	if (bVelocity)
	{
	...
	ID3D12RootSignature* pd3dGraphicsRootSignature ,int nMeshes) :CPlayer(nMeshes)
{
	CMesh* pAirplaneMesh = new CAirplaneMeshDiffused(pd3dDevice, pd3dCommandList, 20.0f,
		4.0f, 20.0f, XMFLOAT4(0.5f, 0.0f, 0.0f, 0.0f));
...
CCamera* CAirplanePlayer::ChangeCamera(DWORD nNewCameraMode, float fTimeElapsed)
{
	...
	case SPACESHIP_CAMERA:
		...
		m_pCamera->SetTimeLag(0.0f);
		m_pCamera->SetOffset(XMFLOAT3(0.0f, 0.0f, -70.0f));
		m_pCamera->GenerateProjectionMatrix(1.01f, 5000.0f, ASPECT_RATIO, 60.0f);
	...
	case THIRD_PERSON_CAMERA:
		...
		SetFriction(250.0f);
		SetGravity(XMFLOAT3(0.0f, 0.0f, -10.0f));
		SetMaxVelocityXZ(125.0f);
		SetMaxVelocityY(400.0f);
...
CTerrainPlayer::CTerrainPlayer(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, void* pContext, int nMeshes) :CPlayer(nMeshes)
{
	...
	CAirplaneMeshDiffused* pCubeMesh = new CAirplaneMeshDiffused(pd3dDevice, pd3dCommandList,4.0f, 12.0f, 4.0f);
	SetMesh(0, pCubeMesh);
