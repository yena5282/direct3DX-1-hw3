	...
	* pd3dCommandList) : CMesh(pd3dDevice, pd3dCommandList)
{
	...
	m_pd3dIndexBuffer = ::CreateBufferResource(pd3dDevice, pd3dCommandList, pVertices,
		m_nStride * m_nVertices, D3D12_HEAP_TYPE_DEFAULT,
		D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, &m_pd3dIndexUploadBuffer);
	// 정점 버퍼 뷰를 생성한다. 
	m_d3dVertexBufferView.BufferLocation = m_pd3dIndexBuffer->GetGPUVirtualAddress();
...
// 직육면체를 삼각형 리스트(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST)로 표현하려면 총 36개의 정점 데이터
// 가 필요하다(6x2x3).즉, 직육면체는 6개의 사각형이 필요하고 각 사각형은 2개의 삼각형, 그리고 각 삼각
// 형은 3개의 정점 데이터가 필요하다.
	...
	float fWidth, float fHeight, float fDepth, XMFLOAT4 xmf4Color) : CMesh(pd3dDevice, pd3dCommandList)
{
	...
	int i=0;
	CDiffusedVertex pVertices[72];
	
	// 비행기 메쉬의 위쪽 면
	pVertices[i++] = CDiffusedVertex(XMFLOAT3(0.0f, +(fy + y3), -fz), Vector4::Add(xmf4Color,
		RANDOM_COLOR));
...
CHeightMapGridMesh::CHeightMapGridMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList
	, int xStart, int zStart, int nWidth, int nLength, XMFLOAT3 xmf3Scale, XMFLOAT4 xmf4Color, void* pContext) : CMesh(pd3dDevice,
		pd3dCommandList)
{
