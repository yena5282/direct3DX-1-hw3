...
class CCamera
{
	// protected : Private 역할을 하면서 상속받은 클래스만 접근하고 싶을 때
protected:
	//카메라의 위치(월드좌표계) 벡터이다. 
...
public:
	...
	virtual void CreateShaderVariables(ID3D12Device* pd3Device, ID3D12GraphicsCommandList* pd3dCommandList);
	...
	void SetViewport(int xTopLeft, int yTopLeft, int nWidth, int nHeight, float gMinZ = 0.0f, float fMaxZ = 1.0f);
